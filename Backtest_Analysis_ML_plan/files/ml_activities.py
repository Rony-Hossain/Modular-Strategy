...
async def claude_plan_ml_phase(context: MLPhaseContext) -> GeminiScriptRequest:
    ...
    """
    runner = ClaudeRunner(timeout_seconds=180, effort="max")
...
- script_name (string)
"""
    return await runner.prompt_json(prompt, GeminiScriptRequest)
...
"""
    try:
        return await _gemini("COMPILER").prompt(prompt)
    except Exception:
        activity.logger.warning("Primary Gemini failed, using fallback.")
        return await _gemini("FALLBACK").prompt(prompt)
...
async def claude_review_ml_script(
    ...
    runner = ClaudeRunner(timeout_seconds=180, effort="high")
...
"""
    return await runner.prompt_json(prompt, ScriptReviewDecision)
...
"""
    return await runner.prompt_json(prompt, PhaseEvaluation)
...
async def build_ml_proposal(
    ...
    """
    runner = ClaudeRunner(timeout_seconds=240, effort="max")
...
"""
    return await runner.prompt_json(prompt, MLProposal)
