...
class _FallbackWrapper(LLMRunner):
    ...
    async def prompt(self, prompt_text: str) -> str:
        try:
            return await self.primary.prompt(prompt_text)
            ...
            return await self.fallback.prompt(prompt_text)

    async def prompt_json(self, prompt_text, schema):
        try:
            return await self.primary.prompt_json(prompt_text, schema)
        except Exception as e:
            print(
                f"[FALLBACK] role={self.role} primary={self.primary.provider_name}"
                f" failed: {type(e).__name__}: {str(e)[:200]} — trying fallback"
            )
            return await self.fallback.prompt_json(prompt_text, schema)
