...
class MLPipelineWorkflow:
    ...
    async def run(self, payload: dict) -> dict:
        ...
        # health is {provider: bool, possibly provider_error: str}.
        # Treat as failed only if at least one bool is False.
        bool_results = {k: v for k, v in health.items() if isinstance(v, bool)}
        unhealthy = [p for p, ok in bool_results.items() if not ok]
        if unhealthy:
            log.error("startup", f"Provider(s) unavailable: {unhealthy}")
            raise ApplicationError(
                f"Provider(s) unavailable: {unhealthy}", non_retryable=True,
            )
    ...
    async def _build_phase(
        ...
        if existing_artifacts:
            ...
            if previews:
                ...
                artifact_previews = "\n".join(preview_lines)
                log.event("ARTIFACT_PREVIEW", phase_id=phase.phase_id,
                          sampled=list(previews.keys()))

        # --- Intelligence: sample dependent scripts for import compatibility ---
        dep_script_snippets = ""
        if phase.dependencies:
            all_phases_map = {p.phase_id: p for p in ML_PHASES}
            dep_names = [
                all_phases_map[dep_id].script_name
                for dep_id in phase.dependencies
                if dep_id in all_phases_map
            ]
            if dep_names:
                snippets = await workflow.execute_activity(
                    sample_dependent_scripts,
                    args=[f"{repo_root}/Analysis/scripts", dep_names, 40],
                    start_to_close_timeout=timedelta(seconds=15),
                )
                if snippets:
                    snippet_lines = []
                    for sname, code in snippets.items():
                        snippet_lines.append(f"=== {sname} (dependency) ===\n{code}")
                    dep_script_snippets = "\n".join(snippet_lines)
                    log.event("DEP_SCRIPTS_SAMPLED", phase_id=phase.phase_id,
                              scripts=list(snippets.keys()))
        ...
        for run_attempt in range(MAX_RUN_RETRIES + 1):
            if run_attempt > 0:
                log.warning(phase.phase_id,
                            f"Retry attempt {run_attempt}/{MAX_RUN_RETRIES}")
