...
- Output: print() with [INPUT], [CHECK], [RESULT], [SAVED] tags — ALL FOUR are REQUIRED in every script. These are RUNTIME prints, NOT bare lines in the source.
- All paths via pathlib.Path.
- The first line of your output must be valid Python (import / # comment / etc).
- NEVER use markdown (**bold**) inside Python strings — it breaks syntax.
- NEVER use & inside f-strings (P&L → PnL or P_and_L).
- NEVER use $ in variable names.
- ALWAYS close every f-string brace: {{variable}} not {{variable.
"""
...
- Print [INPUT], [PARSE], [CHECK], [RESULT], [SAVED] sections — ALL FIVE are REQUIRED.
...
PYTHON STRING SAFETY — these cause SyntaxError and will be rejected:
- NEVER use markdown formatting (**bold**, *italic*) inside Python strings or f-strings.
  BAD:  print(f"**Trades**: {n}")
  GOOD: print(f"Trades: {n}")
- NEVER use & inside f-strings or string literals (P&L breaks strings).
  BAD:  f"P&L: {pnl}"
  GOOD: f"PnL: {pnl}"  or  f"P_and_L: {pnl}"
- ALWAYS close every f-string brace: every {{ opening { must have a closing }}.
  BAD:  f"value: {profit_factor"
  GOOD: f"value: {profit_factor}"
- NEVER use $ in variable names (realized_pnl_$ is invalid Python).
  BAD:  realized_pnl_$ = ...
  GOOD: realized_pnl = ...
