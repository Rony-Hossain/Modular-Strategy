...
class MLPhaseSpec(BaseModel):
    ...
    # Cost optimization: phases with deterministic, templated specs don't
    # need the full Gemini-drafts/Claude-edits planning loop. The plan doc
    # is handed straight to the script writer. Use for bug-hunt phases.
    skip_planning: bool = False

    # Cost optimization: short scripts (typically from skip_planning phases)
    # that pass local validation can skip the Claude review entirely.
    # The validator already catches the high-leverage bugs.
    skip_review_if_short: bool = False
...
ML_PHASES: List[MLPhaseSpec] = [
    ...
    # These phases follow a templated audit pattern. Skip the
    # Gemini-drafts/Claude-edits planning loop and skip review for
    # short scripts that pass local validation.
    MLPhaseSpec(
        phase_id="25",
