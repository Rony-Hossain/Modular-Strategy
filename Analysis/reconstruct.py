...
# Reconstruct OnStateChange + OnBarUpdate
# We replace from State == State.Configure until OnMarketData start

osc_start = text_host.find('else if (State == State.Configure)')
omd_start = text_host.find('protected override void OnMarketData')

if osc_start != -1 and omd_start != -1:
    reconstruction = '''else if (State == State.Configure)
            {
        ...
        '''
    text_host = text_host[:osc_start] + reconstruction + text_host[omd_start:]
    with open(path_host, 'w', encoding='utf-8') as f:
        f.write(text_host)

# --- Fix StrategyLogger.cs LogEvalRow ---
path_log = r'D:\\Ninjatrader-Modular-Startegy\\ModularStrategy\\StrategyLogger.cs'
with open(path_log, 'r', encoding='utf-8') as f:
    text_log = f.read()

import re
pattern = re.compile(r'public void LogEvalRow\(.*?bool v_brick\)\s*\{', re.DOTALL)
replacement = '''public void LogEvalRow(DateTime time, RawDecision d, MarketSnapshot snap, string filterReason, bool wasRankedWinner, bool wasTakenLive, int rawScore, int finalScore, double confMult, double rankScore, int numCands, double spread, string sessionName, int minSinceOpen, string logStatus, bool v_smf, bool v_div, bool v_imbal, bool v_exh, bool v_trap, bool v_ice, bool v_sweep, bool v_brick, double simPnl = 0, string firstHit = null)
        {'''
text_log = pattern.sub(replacement, text_log)

# Also update the string formatting inside LogEvalRow to include simPnl and firstHit
if 'd.Target2Price, 0,\\n                0, simPnl, 0,' not in text_log:
...
print("Full structural reconstruction complete.")
