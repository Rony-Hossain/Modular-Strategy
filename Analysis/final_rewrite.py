...
# We will construct the file from scratch to be 100% sure.
# Wait, scratch might be too risky if I miss something.
# Let's read the file and replace the whole section from OnBarUpdate start to OnMarketData start.
...
# 1. Standardize OnStateChange
# Ensure AddVolumetric is correctly placed in State.Configure
text = re.sub(r'else if \(State == State.Configure\)\s*\{.*?\}', 
r'''else if (State == State.Configure)
            {
...
# 2. Complete rewrite of OnBarUpdate
obu_replacement = '''protected override void OnBarUpdate()
        {
            _evalBuffer.Clear();
...
# Replace from 'protected override void OnBarUpdate()' to 'protected override void OnMarketData'
# We find the start of the next method and replace everything in between.
pattern = re.compile(r'protected override void OnBarUpdate\(\).*?protected override void OnMarketData', re.DOTALL)
text = pattern.sub(obu_replacement + '\n\n        protected override void OnMarketData', text)

with open(path, 'w', encoding='utf-8') as f:
    f.write(text)

print("HostStrategy.cs completely standardized.")
