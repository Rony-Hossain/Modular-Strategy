...
# Find the end of State.Configure try block
target = 'catch (Exception ex) { Print(\"AddVolumetric FAILED: \" + ex.Message); }\\n            }'
pos = text.find(target)
if pos == -1:
    print("Could not find target position")
    # Try alternative
    target = 'catch (Exception ex) { Print(\"AddVolumetric FAILED: \" + ex.Message); }'
    pos = text.find(target)

if pos != -1:
    end_of_configure = pos + len(target)
    # The next thing should be 'else if (State == State.DataLoaded)'
    # Or actually the closing brace of State.Configure, which is already in 'target'.
    
    # We want to insert OnBarUpdate after the OnStateChange method ends.
    # OnStateChange ends after State.Terminated block.
    
    state_terminated_end = text.find('else if (State == State.Terminated) { _ui?.DisposeResources(); _log?.Dispose(); }')
    if state_terminated_end != -1:
        # Find the next closing brace
        method_end = text.find('}', state_terminated_end)
        if method_end != -1:
            insertion_point = method_end + 1
            
            # The current file has a mess after OnStateChange. 
            # We want to replace everything from method_end + 1 until OnMarketData.
            
            market_data_start = text.find('protected override void OnMarketData')
            if market_data_start != -1:
        ...
        '''
                text = text[:insertion_point] + new_obu + text[market_data_start:]

with open(path, 'w', encoding='utf-8') as f:
    f.write(text)

print("HostStrategy.cs fixed surgically.")
