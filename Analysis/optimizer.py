...
def load_trades(log_path: Path) -> pd.DataFrame:
    ...
    score_map = sig.set_index('_key')[['_score','_grade','_accepted','_src']].to_dict('index')

    # ── TOUCH_OUTCOME rows ────────────────────────────────────────────────────
    touch = raw[raw['Tag'] == 'TOUCH_OUTCOME'].copy()
    log(f"  SIGNAL_ACCEPTED: {sig['_accepted'].sum()}  "
    ...
    for _, r in touch.iterrows():
        ...
        row = dict(
            ...
            h4_aligned  = 1 if (is_long and h4 > 0) or (not is_long and h4 < 0) else
                         -1 if (is_long and h4 < 0) or (not is_long and h4 > 0) else 0,
            h2_aligned  = 1 if (is_long and h2 > 0) or (not is_long and h2 < 0) else
                         -1 if (is_long and h2 < 0) or (not is_long and h2 > 0) else 0,
            h1_aligned  = 1 if (is_long and h1 > 0) or (not is_long and h1 < 0) else
                         -1 if (is_long and h1 < 0) or (not is_long and h1 > 0) else 0,
    ...
    log(f"  Parsed {len(df)} outcome rows  "
        f"({df['win'].sum()} wins / {(df['win']==0).sum()} losses)  "
        f"win%={df['win'].mean()*100:.1f}%")
    log(f"  Accepted: {(df['rejected']==0).sum()}  Rejected (simulated): {df['rejected'].sum()}")
    log(f"  Net simulated PnL (all): ${df['sim_pnl'].sum():,.0f}  "
...
SNAP_FEATURES = [
    'snap_BD','snap_CD','snap_ABS','snap_SBULL','snap_SBEAR',
    'snap_H1','snap_H2','snap_H4','snap_REG','snap_STR','snap_DEX',
    'snap_BDIV','snap_BERDIV','snap_SW','snap_TRD','snap_ATR','snap_HASVOL',
    'h4_aligned','h2_aligned','h1_aligned','bd_aligned','cd_aligned',
]
...
def ml_feature_analysis(df: pd.DataFrame) -> dict:
    ...
    rf.fit(X, y)
    importances = sorted(zip(SNAP_FEATURES, rf.feature_importances_), key=lambda x: -x[1])
    ...
    for feat, imp in importances[:10]:
        ...
        direction = "-> WIN" if corr > 0.05 else "<- LOSS" if corr < -0.05 else "neutral"
    ...
    gb_scores = cross_val_score(gb, X, y, cv=StratifiedKFold(5, shuffle=True, random_state=42),
                                scoring='roc_auc')
    log(f"\n  GradientBoosting CV AUC: {gb_scores.mean():.3f} +/- {gb_scores.std():.3f}")

    # Per-source analysis
    log("\n  Per-Source Performance:")
    log(f"  {'Source':<25} {'N':>5} {'Win%':>6} {'AvgPnL':>8} {'AvgMFE':>8} {'AvgMAE':>8} {'EV':>8}")
    source_stats = {}
    for src, g in df[df['rejected']==0].groupby('source'):
        ...
        amae = g['mae'].mean()
        ev   = g['sim_pnl'].sum()
        source_stats[src] = dict(n=n, win_rate=wr, avg_pnl=apnl, mfe=amfe, mae=amae, ev=ev)
        log(f"  {src:<25} {n:>5} {wr:>5.1f}% {apnl:>8.1f} {amfe:>8.0f} {amae:>8.0f} {ev:>8.0f}")

    return dict(importances=importances, source_stats=source_stats, rf_auc=cv_scores.mean())
...
def _simulate(df: pd.DataFrame, thresholds: dict, global_thresh: int,
              min_rr: float = 1.0, require_h4: bool = False) -> dict:
    ...
    return dict(n=len(taken), pnl=total, win_rate=win_rate,
                sharpe=sharpe, max_dd=max_dd, avg_pnl=pnls.mean())
...
def optimize_thresholds(df: pd.DataFrame, n_trials: int = 300) -> dict:
    ...
    log(f"  Baseline (threshold=60, all accepted): "
    ...
    optimized = _simulate(df, per_src, best['global_thresh'],
    ...
    return dict(global_thresh=best['global_thresh'],
                require_h4=best['require_h4'],
                per_source=per_src,
                metrics=optimized,
                baseline=baseline)
...
def monte_carlo(df: pd.DataFrame, n_sims: int = 10_000) -> dict:
    ...
    expected_max_streak = math.log(n) / math.log(1 / (1 - wr_actual)) if wr_actual < 1 else 0
    log(f"    Expected max streak (theory): {expected_max_streak:.1f}")

    return dict(ruin_prob=ruin_prob,
...
def write_optimized_config(cfg_path: Path, threshold_result: dict,
    ...
    for k in ['LAYER_A_H4','LAYER_A_H2','LAYER_A_H1',
              'PENALTY_H4','PENALTY_H2','PENALTY_BOTH_EXTRA',
              'PROXY_BAR_DELTA','PROXY_REGIME',
...
def main():
    ...
    if ml_result.get('source_stats'):
        ...
        sorted_src = sorted(ml_result['source_stats'].items(), key=lambda x: x[1]['avg_pnl'])
        for src, s in sorted_src[:3]:
            log(f"    {src:<25}: win={s['win_rate']:.1f}%  avg=${s['avg_pnl']:.1f}  "
                f"consider raising threshold or removing")
        log("\n  Strongest sources:")
        for src, s in sorted(ml_result['source_stats'].items(),
                              key=lambda x: -x[1]['avg_pnl'])[:3]:
            log(f"    {src:<25}: win={s['win_rate']:.1f}%  avg=${s['avg_pnl']:.1f}  keep/lower threshold")
