...
# 1. Fix OnStateChange (ensure it is clean)
# Find the start of the try block in OnStateChange
osc_try_start = text.find('AddDataSeries(Data.BarsPeriodType.Minute, 240);')
osc_try_end = text.find('else if (State == State.DataLoaded)')

if osc_try_start != -1 and osc_try_end != -1:
    clean_osc = '''AddDataSeries(Data.BarsPeriodType.Minute, 240);
    ...
    text = text[:osc_try_start] + clean_osc + '\n            ' + text[osc_try_end:]

# 2. Fix OnBarUpdate (complete rewrite of the method logic)
obu_start = text.find('protected override void OnBarUpdate()')
obu_end = text.find('protected override void OnMarketData(MarketDataEventArgs e)')

if obu_start != -1 and obu_end != -1:
    # Find the closing brace of OnBarUpdate
    # It should be before OnMarketData
    real_obu_end = text.rfind('}', obu_start, obu_end)
    
    clean_obu = '''protected override void OnBarUpdate()
        {
            _evalBuffer.Clear();
    ...
    text = text[:obu_start] + clean_obu + '\n\n        ' + text[real_obu_end+1:]

with open(path, 'w', encoding='utf-8') as f:
    f.write(text)

print("HostStrategy.cs cleanly rewritten.")
